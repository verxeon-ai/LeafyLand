...
/** 
 * Calculate dynamic shipping logic based on commerce type (Retail vs B2B Bulk).
 * @param {string} commerceType - 'B2C' or 'B2B'
 * @param {number} quantity - Number of items
 * @param {number} distanceKm - Delivery distance
 */
export function calculateDynamicShipping(commerceType, quantity, distanceKm = 50) {
    if (commerceType === 'B2B' || quantity > 100) {
        // LTL (Less Than Truckload) or Freight Logic
        const baseFreight = 5000 * 100; // Paise
        const perKmRate = 15 * 100; // Paise
        return baseFreight + (distanceKm * perKmRate);
    }
    // Standard retail shipping
    if (distanceKm < 10) return 50 * 100;
    if (distanceKm < 50) return 150 * 100;
    return 300 * 100;
}

/**
 * Escrow-like payment logic for B2B/Tenders (milestone based).
 * We create Razorpay orders but track the milestone in notes.
 */
export async function createEscrowMilestonePayment(entityId, milestoneName, amountPaise) {
    const notes = {
        paymentType: 'ESCROW_MILESTONE',
    ...
    return createRazorpayOrder({
        amountPaise,
        receipt: `escrow_${entityId}`.slice(0, 40),
        notes
    });
}
