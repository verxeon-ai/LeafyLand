...
export async function notifyCodOrders(batch) {
    ...
    try {
        ...
        await notifyAllAdmins({
            type: 'ORDER_PLACED',
            title: batch.orders.length === 1 ? 'New COD order' : `${batch.orders.length} new COD orders`,
            body: batch.orders.map((order) => order.store?.name).filter(Boolean).join(', ') || 'A customer placed a cash-on-delivery order.',
            link: '/admin/orders',
        })
    } catch (e) {
        console.error('COD order notifications failed:', e?.message || e)
    }
}
