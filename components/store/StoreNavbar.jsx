...
const StoreNavbar = ({ onMenuToggle, storeInfo }) => {
    ...
    const storeName = storeInfo?.name?.trim() || 'Store'
    const settingsActive = pathname === '/store/settings'

    const submitSearch = (e) => {
        e.preventDefault()
        window.dispatchEvent(new CustomEvent('leafyland-vendor-search', { detail: query }))
    }
        ...
        <header className="sticky top-0 z-50 shrink-0 border-b border-gray-200 bg-white">
            <div className="flex h-14 min-w-0 items-center gap-2 px-3 sm:h-[4.25rem] sm:gap-3 sm:px-4 lg:px-6">
                ...
                <div className="ml-auto flex shrink-0 items-center">
                    ...
                    <Link
                        href="/store/settings"
                        className={`${navBtnClass} ${settingsActive ? 'bg-[#eef4ef]' : ''}`}
                        style={{ color: BRAND_TEXT }}
                        title={storeName}
                        aria-label="Open store settings"
                    >
                        <Store size={20} strokeWidth={1.75} />
                        <span className="hidden max-w-[9rem] truncate whitespace-nowrap text-[10px] font-medium lg:block" style={{ color: BRAND_MUTED }}>
                            {storeName}
                        </span>
                    </Link>
            ...
            <form
                onSubmit={submitSearch}
                className="flex h-11 items-stretch overflow-hidden rounded-xl border border-gray-200 bg-white mx-3 mb-3 focus-within:border-[#2f7d4a] focus-within:ring-1 focus-within:ring-[#2f7d4a]/20 lg:hidden"
            >
                <input
                    className="min-w-0 flex-1 px-3 text-sm outline-none placeholder:text-gray-400"
                    style={{ color: BRAND_TEXT }}
